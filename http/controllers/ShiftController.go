...
func GetAllShifts(c *gin.Context) {
	...
	type shiftRow struct {
		models.Shift
		UserOpen string `json:"user_open"`
		UserClose string `json:"user_close"`
		StoreName string `json:"store_name"`
		ExpectedCash float64 `json:"expected_cash"`
	}
	...
	baseQuery := config.DB.Table("shifts s").
		Joins("LEFT JOIN users u_open ON u_open.id = s.open_by").
		Joins("LEFT JOIN users u_closed ON u_closed.id = s.closed_by").
		Joins("LEFT JOIN store_profiles sp ON sp.id = s.store_id").
	...
	if store_id != "" {
		...
		baseQuery.Where("s.store_id = ?", store.ID)
    }
	
	if q != "" {
		like := "%" + q + "%"
		baseQuery.Where("(u_open.name LIKE ? OR u_closed.name LIKE ? OR sp.store_name LIKE ?)", like, like, like)
	}

	var totalData int64
	if err := baseQuery.Session(&gorm.Session{}).Count(&totalData).Error; err != nil {
		helpers.ErrorResponse(c, 500, "Gagal menghitung total shift", err)
		return
	}

	if err := baseQuery.Select(`
		s.*,
		COALESCE(u_open.name, "-") as user_open,
		COALESCE(u_closed.name, "-") as user_close,
		COALESCE(sp.store_name, "-") as store_name,
		COALESCE(s.total_cash + s.initial_cash, 0) as expected_cash
	`).Limit(limit).Offset(offset).Order("s.created_at desc").Scan(&rows).Error; err != nil {
		helpers.ErrorResponse(c, 500, "Gagal mengambil data shift", err)
		return
	}
