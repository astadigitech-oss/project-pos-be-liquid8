...
func ListMembersStore(c *gin.Context) {
    ...
    baseWhere := fmt.Sprintf("WHERE store_id = %d", *user.StoreID)
    args := []interface{}{}
    if q != "" {
    ...
    countSQL := fmt.Sprintf("SELECT COUNT(*) FROM members %s", baseWhere)
    if len(args) > 0 { 
		if err := config.DB.Raw(countSQL, args...).Scan(&total).Error; err != nil { helpers.ErrorResponse(c, 500, "Failed to count members", err); return }
	} else {
		if err := config.DB.Raw(countSQL).Scan(&total).Error; err != nil { helpers.ErrorResponse(c, 500, "Failed to count members", err); return }
	}
